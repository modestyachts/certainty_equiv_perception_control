...
class polynomial_observations():
    ...
    def __init__(self, pows, sv=0, q=100, q_lim=20):
        ...
        self.indicator_powers = lambda y: [np.power(indicator_observe(y, lim=(-q_lim,q_lim)),1/p) for p in pows]
...
def create_list_X_for_tracker(Xs):
    X = []
    for xs in zip(*Xs):
        X.append(np.hstack(xs))
    return X
